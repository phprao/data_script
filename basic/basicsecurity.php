<?php
	...
	class basicsecurity {
		...
		public static $key_table_value = array
		(
		...
		public static function decryption($key, $data) {
			$index = ((int)$key)%1024;
			$len = count($data) - 1;
			$key_value = basicsecurity::$key_table[$index];
			for($i = $len; $i > 0; $i--) {
				$data[$i] = $data[$i] ^ $data[$i - 1];
			}
			$data[0] = $key_value^$data[0];
			return $data;
		}
		...
		public static function get_bytes_array($string) { 
			/*$bytes = array(); 
		...
		public static function encryp_data($key,$data) {
			...
			$var_byte = basicsecurity::get_bytes_array((string)$data);

			//$var_data = basicsecurity::encryption((int)$key,&$var_byte);
			//encry 
			$var_byte = basicsecurity::encryption((int)$key,$var_byte);
		
			//byte to string
			$var_data = basicsecurity::bytes_to_string($var_byte);
			// string to base64
			return base64_encode($var_data);
		}

		public static function decryp_data($key,$data) {
			if(null == $data) {
				return "";
			}

			//base64 to string 
			$var_data = base64_decode((string)$data,true);

			//echo "111";
			//var_dump($var_data);
			//string to byte array 
			$var_byte = basicsecurity::get_bytes_array($var_data);
			//echo "222";
			//var_dump($var_byte);
			//decryp
			$var_byte = basicsecurity::decryption((int)$key,$var_byte);
			//echo "333";
			//var_dump($var_byte);
			//byte to string
			$value = basicsecurity::bytes_to_string($var_byte);
		...
		public static function get_key_value($keys) {
			$index = ((int)$keys) % 256;
			$key_value = basicsecurity::$key_table_value[$index];
			return (string)$key_value;
		}
		
		public static function repall_str($data) {
		   return str_replace('_','+',$data);
		}
		
		public static function encryp_data_ex($key,$data) {
			$key_value = basicsecurity::get_key_value($key);
			$str = basicsecurity::rc4($key_value, $data);
			$value = base64_encode($str);
			return $value;
		}
		
		public static function decryp_data_ex($key,$data) {
			$str_data = basicsecurity::repall_str($data);
			$data_value = base64_decode((string)$str_data);
			$key_value = basicsecurity::get_key_value($key);
			$value = basicsecurity::rc4($key_value, $data_value);
			return $value;
