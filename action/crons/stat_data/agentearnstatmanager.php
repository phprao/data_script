<?php
	...
	class agentearnstatmanager {
		...
		protected function update_promoters_statistics($update_data){
			...
			$res = $this->get_agents_relation($agentmode,$this->agent['agent_id']);
			if(!$res){
				BASIC_LOG_ERROR($this->logtag,'%s',"get_agents_relation代理信息有误");
				return false;
			}
			...
			// BASIC_LOG_ERROR($this->logtag,'%s',"agents_relation --------- ".json_encode($this->agents_relation));
			...
			foreach($this->agents_relation as $k => $v){
				...
				if($v['agent_login_status'] != 1){
					continue;
				}
				if($v['agent_login_status_man'] == 1){
					// 人为开启的星级
					$task = new configdatatask();
			        $task->set_action('select');
			        $task->append_where(array('config_name'=>'star_income_limit_num','config_status'=>1));
			        $result = $this->app->m_server->process_database($task, null, null, null);
			        if(!$result){
			            $star_income_limit_num = 10;
			        }else{
			        	$config = json_decode($result->get('config_config', ''), true);
			        	$star_income_limit_num = $config['agent_promote_count'];
			        }
			        if($v['agent_promote_count'] < $star_income_limit_num){
			        	continue;
			        }
				}
				// 查询是否存在
		...
		protected function agent_get_rate(){
	        ...
	        if(!$data){
	        	BASIC_LOG_ERROR($this->logtag,'%s',"agent_get_rate代理信息有误");
	        }
