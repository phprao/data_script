<?php
	 ...
	 * describe: 游戏记录处理
	 +---------------------------------------------------------- 
	 */
	class gamerecordstat extends basicaction{
		...
		protected function logic(basicdi $app) {
			$this->app = $app;
			$list = $this->get_list();
			if(is_null($list) || empty($list)) return false;
			
			foreach($list as $val){
				$this->record_data($val);
			}

			return true;
		}

		protected function record_data($data) {
			if(!$data){
				return true;
			}
			...
			*/
			...
			$trank = new basictransactiontask();
			$this->app->m_server->process_database($trank,null,null,null);
			$ret = stat_gamebeatrecord::get_instance()->record_data($this->app,$data);
			if($ret){
				$trank->commit();
				return true;
			}else{
				$trank->rollback();
				// 更新时间
				$this->update_log_time($data);
				$this->set_response_status(1111, '记录处理失败');
				return false;
			}
		}
		...
		protected function get_list() {
			...
			$where = ['game_record_update_time' => time() - $this->time_out];
			$list = $recordmodel->get_list_model($where, $other);
			return $list;
		}

		protected function update_log_time($data){
			$recordmodel = new gamerecordmodel($this->app);
			$recordmodel->update_log_time($data);
		}
