...
class playerproplogdaydatatask extends basicdatatask {
    ...
    public function __construct()
    {
        ...
        basicfields::playerproplogday_fields($this);

        $this->set_data_table_info('dc_player_prop_log_day', 'log_day_id');
    }
   ...
   public function set_extra_action($extra_action)
   {
    ...
    protected function query_available_to_other_emoticons(basicmysql $db, $param)
    {
        ...
        $db_task_emoticon = new propdatatask();
        ...
        if ($player_vip_level) {    // 玩家为VIP
            $is_vip = 1;
            $list = $this->_query_vip_available_emoticons($db, $table_config, $param);
        } else {    // 普通玩家或游客
            $is_vip = 0;
            $list = $this->_query_normal_available_emoticons($db, $table_config, $param);
        }

        $return = array('is_vip'=>$is_vip, 'prop_data'=>$list);
        return $return;
    }
     ...
     * @author Zhanghui
     */
    private function _query_vip_available_emoticons(basicmysql $db, array $table_config, array $param)
    {
        $table_emoticon = $table_config['table_emoticon'];
        $limit = $table_config['limit'];
        $player_vip_level = $param['self_player_vip_level'];
        $fields = "`prop_id`,`prop_price`,`prop_specific_config`";
        $where = "WHERE `prop_category`=1 AND `prop_vip_level`<={$player_vip_level}";
        $orderby = "ORDER BY `prop_apply_group` DESC";                      // 优先排序VIP表情
        $sql = "SELECT {$fields} FROM `{$table_emoticon}` {$where} {$orderby} {$limit}";
        ...
        if ($list) {
            foreach ($list as $key=>$value) {
                $return[$key]['prop_id'] = $value['prop_id'];
                $return[$key]['prop_price'] = $value['prop_price'];
                $return[$key]['prop_available_times'] = -1;                 // 不限制次数
                $return[$key]['prop_limit_times'] = -1;
            }
        }
    ...
    private function _query_normal_available_emoticons(basicmysql $db, array $table_config, array $param)
    {
        ...
        $fields = "A.`prop_id`,A.`prop_price`,A.`prop_specific_config`,B.*";
        $join = "ON A.`prop_id`=B.`log_day_prop_id`";
        $where = "WHERE A.`prop_category`=1 AND A.`prop_apply_group`=0";
        $fields_sub_query = "`log_day_prop_id`,IFNULL(`log_day_prop_consumed_num`, 0) AS `log_day_prop_consumed_num`";
        $where_sub_query = "WHERE `log_day_player_id`={$player_id} AND `log_day_date`='{$date}'";
        ...
        if ($result) {
            foreach ($result as $key=>$value) {
                $list[$key]['prop_id'] = $value['prop_id'];
                $list[$key]['prop_price'] = $value['prop_price'];

                $temp_prop_specific_config = json_decode($value['prop_specific_config'], true);
                if (!is_null($temp_prop_specific_config) && $temp_prop_specific_config['limit_times'] != -1) {     // 有配置则获取可用次数
                    $tmp_prop_available_limit_times = $temp_prop_specific_config['limit_times'] - $value['log_day_prop_consumed_num'];
                    $list[$key]['prop_available_times'] = $tmp_prop_available_limit_times > 0 ? $tmp_prop_available_limit_times : 0;
                    $list[$key]['prop_limit_times'] = $temp_prop_specific_config['limit_times'];
                } else {    // 没有配置 默认为无限制
                    $list[$key]['prop_available_times'] = -1;
                    $list[$key]['prop_limit_times'] = -1;
                }
            }
        }
